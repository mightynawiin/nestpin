...
function MapController({ center, zoom, radius = 10, liveLocation = null }) {
  const map = useMap();
  useEffect(() => {
    if (!center) return undefined;

    const isDefaultCenter = center[0] === DEFAULT_CENTER[0] && center[1] === DEFAULT_CENTER[1];
    if (isDefaultCenter && !liveLocation) {
      map.setView(center, zoom || map.getZoom(), { animate: true });
      return undefined;
    }

    const safeRadius = Math.max(Number(radius) || 1, 1);
    const padding = Math.min(120, Math.max(26, safeRadius * 2.4));
    const bounds = L.latLng(center).toBounds(safeRadius * 1000);
    map.fitBounds(bounds, { padding: [padding, padding], maxZoom: 15, animate: true });

    const locate = event => {
      const next = event.detail;
      if (Array.isArray(next) && next.length === 2) {
        window.setTimeout(() => map.flyTo(next, 15, { animate: true }), 80);
      }
    };

    window.addEventListener('nestpin:locate', locate);
    return () => window.removeEventListener('nestpin:locate', locate);
  }, [center, zoom, radius, liveLocation, map]);
...
function LocationPanel({ user, existing, onComplete }) {
  ...
  const save = async (event) => { event.preventDefault(); if (!name.trim() || !location) return setError('Add your name and select a location.');
    const { error: saveError } = await supabase.from('profiles').upsert({ id: user.id, display_name: name.trim(), location_name: query || 'Selected map location', latitude: location[0], longitude: location[1] });
  ...
  return <div className="location-overlay"><div className="location-card"><div className="step-count">01 / 01</div><h2>Where are you looking?</h2><p className="muted">Set your home base so Nestpin can show relevant places around you.</p>
    <form onSubmit={save}><label>Your display name<input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. maya_r" /></label><label>Your area<div className="search-field"><Search size={17} /><input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), search())} placeholder="Search city or region" /><button type="button" onClick={search}>{searching ? <LoaderCircle className="spin" size={17} /> : 'Find'}</button></div></label><p className="helper">Search a place, or click anywhere on the map to fine-tune your area.</p>{error && <div className="error">{error}</div>}<button className="button primary wide" type="submit">Start exploring <MapPin size={17} /></button></form>
  </div></div>;
}
...
function App() {
  ...
  useEffect(() => { supabase.auth.getSession().then(({ data }) => { const nextSession = data.session; setSession(nextSession); if (nextSession) { if (window.location.pathname !== NESTPIN_PATH) navigate(NESTPIN_PATH); load(nextSession.user); } else { if (window.location.pathname !== HOME_PATH) navigate(HOME_PATH); setLoading(false); } }); const { data: listener } = supabase.auth.onAuthStateChange((_event, next) => { setSession(next); if (next) { if (window.location.pathname !== NESTPIN_PATH) navigate(NESTPIN_PATH); load(next.user); } else { if (window.location.pathname !== HOME_PATH) navigate(HOME_PATH); setProfile(null); setLoading(false); } }); return () => listener.subscription.unsubscribe(); }, []);
  ...
  const runSearch = async e => { e.preventDefault(); if (!search.trim()) return; try { const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(search)}`); const data = await res.json(); if (data[0]) { const point = [+data[0].lat, +data[0].lon]; setCenter(point); if (!profile) setPendingPin(point); } else setNotice('No matching place found'); } catch { setNotice('Search is unavailable right now'); } setTimeout(() => setNotice(''), 3000); };
  const remove = async item => { if (!confirm('Remove this listing?')) return; const { data: photos } = await supabase.from('listing_photos').select('storage_path').eq('listing_id', item.id); if (photos?.length) await supabase.storage.from('listing-photos').remove(photos.map(p => p.storage_path)); await supabase.from('listings').delete().eq('id', item.id); setListings(listings.filter(l => l.id !== item.id)); setSelected(null); };
  ...
  if (path !== NESTPIN_PATH || !session) return <AuthPanel onAuthed={(user, name) => { setSession({ user }); setLoading(false); navigate(NESTPIN_PATH); }} />;
  if (!profile) return <><MapContainer center={center} zoom={3} className="map"><TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><MapController center={center} zoom={12} radius={radius} liveLocation={liveLocation} /><MapClick enabled onPick={onLocationPick} />{pendingPin && <Marker position={pendingPin} icon={mineIcon} />}</MapContainer><LocationPanel user={session.user} existing={null} onComplete={p => { setProfile(p); setCenter([p.latitude, p.longitude]); load(session.user); }} /></>;
